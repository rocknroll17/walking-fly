...
POSE_JSON = fm.BUILD_DIR / "stand_pose.json"
...
class Kinematics:
    """Helper bundling the ids needed to evaluate a candidate pose."""
    ...
    def __init__(self, m: mujoco.MjModel):
        ...
        for j in LEG_JOINTS:
            l = name(mujoco.mjtObj.mjOBJ_JOINT, f"{j}_T3_left")
            r = name(mujoco.mjtObj.mjOBJ_JOINT, f"{j}_T3_right")
            self.jq[j] = (m.jnt_qposadr[l], m.jnt_qposadr[r], m.jnt_range[l].copy())
        self.claws = [name(mujoco.mjtObj.mjOBJ_SITE, f"claw_T3_{s}") for s in ("left", "right")]
        self.tarsi = [name(mujoco.mjtObj.mjOBJ_SITE, f"tarsus_T3_{s}") for s in ("left", "right")]
        gname = lambda g: mujoco.mj_id2name(m, mujoco.mjtObj.mjOBJ_GEOM, g) or ""  # noqa: E731
        foot = [g for g in range(m.ngeom) if "T3" in gname(g)]
    ...
    def qpos(self, pitch: float, q_leg: np.ndarray, height: float = 0.0) -> np.ndarray:
        ...
        qpos[:3] = (0, 0, height)
        qpos[3:7] = (np.cos(-pitch / 2), 0, np.sin(-pitch / 2), 0)  # nose-up
