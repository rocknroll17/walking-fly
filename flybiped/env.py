...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        walk_gate_time=0.2,    # s without middle/hind-leg or body contact before progress/goal rewards count (kills kick-and-glide)
    )
...
class FlyBiped(mjx_env.MjxEnv):
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        info = {
            ...
            "biped_streak": jp.zeros(()),      # steps since the last middle/hind-leg or body contact
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        # Steps since the last real kick: a middle/hind leg or the body touching the floor. Wobbles of the
        # standing test (near-touch, height dip, both front feet briefly up) do NOT reset it.
        kicked = contacts["fore"] | contacts["body"]
        streak = jp.where(kicked, 0.0, state.info["biped_streak"] + 1.0)
