...
- 환경 변수로 조절: `CUDA_VISIBLE_DEVICES`(GPU 선택), `FLYBIPED_NUM_ENVS`(병렬 환경 수, 24GB GPU면 8192), `XLA_PYTHON_CLIENT_MEM_FRACTION`(JAX 메모리 비율, 기본 0.45), `FLYBIPED_EVAL_EVERY`(체크포인트 간격 스텝, 기본 500000), `FLYBIPED_GRAPH_MODE`(Warp 그래프 캡처: 기본 `NONE`, 더 빠르지만 검증 필요한 `WARP_STAGED`), `FLYBIPED_BUDGET`(총 스텝 예산, 기본 무제한: 멈출 때까지 계속), `FLYBIPED_CHUNK`(구간 길이, 기본 1e7)
